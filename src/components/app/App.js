...
class App extends Component {
	constructor() {
		...
		this.state = {
			todoData: [this.createItem('Перенести задачи'), this.createItem('Изменить состояния'), this.createItem('Реализовать удаление')],
			filter: 'all',
		};
	}
	...
	deleteCompleted = () => {
		this.setState(state => ({
			todoData: state.todoData.filter(el => !el.done),
		}));
	};
	...
	toggleProp = (arr, id, propName, prop2Name) => {
		...
		const newItem = { ...oldItem, [propName]: !oldItem[propName], [prop2Name]: !oldItem[prop2Name] };
	...
	render() {
		...
		return (
			<div className='todoapp'>
				...
				<Footer doneItems={doneCount} onDeleteCompleted={this.deleteCompleted} filter={filter} onFilterSelect={this.onFilterSelect} />
			</div>
		);
	}
}
